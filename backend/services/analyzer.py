...
- ВСЕГДА находи минимум 2-3 момента. Если видео длинное — моментов будет больше.
...
Выбери до {max_moments} лучших моментов для шортсов. Каждый момент от {min_dur} до {max_dur} секунд. Бери только реально интересные, вирусные, цепляющие моменты. Если нашёл меньше {max_moments} — ничего, качество важнее количества. Шортсы могут быть короткими (15-20 сек) если момент яркий.
...
class MomentAnalyzer:
    ...
    async def _analyze_ollama_chunked(
        self, segments: List[TranscriptSegment],
        ...
        while chunk_start < total_dur:
            ...
            try:
                ...
                try:
                    ...
                    # Попытка 2: retry Ollama
                    logger.warning(f"Ollama чанк {chunk_idx}: не-JSON, retry...")
                    raw = await self._call_ollama(user_prompt)
                    try:
                        data = self._parse_json_response(raw)
                    except (ValueError, json.JSONDecodeError):
                        # Попытка 3: fallback на OpenAI
                        if settings.openai_api_key:
                            logger.warning(f"Ollama чанк {chunk_idx}: fallback на OpenAI")
                            fallback = AsyncOpenAI(api_key=settings.openai_api_key)
                            ...
                            logger.warning(f"Ollama чанк {chunk_idx}: пропуск (нет JSON)")
                            chunk_start = chunk_end
                            chunk_idx += 1
                            continue
    ...
    async def analyze(
        ...
        if self.provider == "ollama":
            raw = await self._analyze_ollama_chunked(
                segments, max_moments, min_duration, max_duration, detected_lang,
            )
        else:
            transcript = self._format_transcript(segments)
            if len(transcript) > 80000:
                transcript = self._format_transcript(segments[::2])
                logger.warning("Транскрипт обрезан для экономии токенов")

            user_prompt = USER_PROMPT_TEMPLATE.format(
                transcript=transcript, max_moments=max_moments,
                min_dur=min_duration, max_dur=max_duration, detected_lang=detected_lang,
            )
            logger.info(f"Отправка транскрипта на анализ (символов: {len(transcript)})")
        ...
        for m in moments_data:
            ...
            if duration < min_duration:
                ...
                duration = end - start
            # Слишком длинные — обрезаем вместо пропуска
